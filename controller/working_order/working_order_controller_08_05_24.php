...
class delivery_noteController
{
    ...
    function __construct()
	{
        ...
        $this->location_txt = $_POST['v_location_txt'];
    ...
    { 
        $array =  array();
         ...
         $array[2] = "CALL proc_generate_working_orderr('".$this->company_id."','".$this->company_name."','".$this->project_id."','".$this->project_name."','".$this->quotation_id."','".$this->quotation_no."','".$this->location_txt."','".$this->received."','".$this->v_working_date."','".$this->working_end_date."','".$this->user_id."','".$this->user_name."','".$this->created_date."','".$this->default_date."',@msg)";
		 ...
		 $array[6]="SELECT wot.*, woct.quotation_status, DATE_FORMAT(wot.work_order_start_date, '%d-%m-%Y') AS work_order_date FROM work_order_tbl wot INNER JOIN ( SELECT work_order_id, MAX(work_order_child_id) AS work_order_child_id FROM work_order_child_tbl GROUP BY work_order_id) AS wocg ON wot.work_order_main_id = wocg.work_order_id INNER JOIN work_order_child_tbl woct ON wocg.work_order_child_id = woct.work_order_child_id WHERE wot.work_order_status = 'Generated' ORDER BY wot.work_order_main_id DESC;";
		 $array[7] = "update work_order_tbl set work_order_status='Cancelled' where work_order_main_id='".$this->v_work_order_main_id."'";
		 $array[8] = "select work_order_main_id,work_order_status,work_order_number,quotation_reference,DATE_FORMAT(work_order_start_date, '%d-%m-%Y') as work_order_date from work_order_tbl where work_order_status='Generated' and DATE_FORMAT(work_order_start_date,'%m/%d/%Y') between '".$this->start_date."' and '".$this->end_date."'";
		 $array[9] = "SELECT work_order_main_id,work_order_number,quotation_reference,DATE_FORMAT(work_order_start_date,'%d-%m-%Y') as work_order_date,work_order_status,company_id,company_name,po_box,telephone_no,fax,attn,project_id,location,received,work_order_status  FROM work_order_tbl where work_order_status='Cancelled'";
		 $array[10] = "select work_order_main_id,work_order_status,work_order_number,quotation_reference,DATE_FORMAT(work_order_start_date, '%d-%m-%Y') as work_order_date from work_order_tbl where work_order_status='Cancelled' and DATE_FORMAT(work_order_start_date,'%m/%d/%Y') between '".$this->start_date."' and '".$this->end_date."'";
		 ...
		 $array[13] = "update work_order_tbl set company_id = '".$this->company_id."', company_name= '".$this->company_name."',project_id='".$this->project_id."',project_name='".$this->project_name."',received='".$this->received."',location='".$this->location_txt."',quotation_reference='".$this->quotation_no."' where work_order_number = '".$this->v_working_order_no."' ";
