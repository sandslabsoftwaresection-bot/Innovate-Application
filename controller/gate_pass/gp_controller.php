...
class gatepasscontrollerController
{
        ...
        public $v_attn, $v_start_date, $v_end_date, $v_project_id, $v_project_name, $v_inventory_id, $v_inventory_name,$v_item_id,$array_table_data;
        public $job_no_id,$v_job_no, $v_location, $v_note;
...
// 		 $array[15] = "SELECT *, (store2_quantity_in - store2_quantity_out) AS Stock FROM inventory_tbl WHERE inventory_category = '".$this->v_category_name."' AND (store2_quantity_in - store2_quantity_out) > 0.00 ";
         $array[15] = "SELECT *, SUM(quantity - qty_out) AS Stock FROM purchase_recieved_child_tbl WHERE inventory_id = '".$this->v_item_id."'";
         ...
         $array[18] = "CALL proc_generate_new_gp('".$this->company_id."','".$this->company_name."', '".$this->v_project_id."','".$this->v_project_name."','".$this->v_gate_pass_date."','".$this->v_vehicle_no."','".$this->v_driver_name."','".$this->v_approved_by."','".$this->v_checked_by."','".$this->v_received_by."','".$this->v_inventory_id."','".$this->v_inventory_name."','".$this->v_description."','".$this->v_qty."','".$this->v_unit."','".$this->v_pass_no."','".$this->v_job_no."','".$this->job_no_id."','".$this->v_location."','". $this->v_note."',@msg,@msg1)";
    ...
    function RequestAccept($FunctionEvents)
    {
        ...
        switch ($FunctionEvents)
        {
			...
			$data = json_decode($this->msg, true);
			 ...
			 $SQLString = "INSERT INTO purchase_recieved_child_tbl(store_category_id, store_category, inventory_id, inventory_name, description, qty_out, unit, item_code, recieved_date, company_id, company_name, project_id, project_name, entry_type, ref_no, master_ref_id) VALUES ";
				  $SQLOther = "";
			    for($i=0;$i<=sizeof($this->array_table_data)-1;$i++){
			
			      $SQLOther = $SQLOther ."('".$this->array_table_data[$i]['CategoryID']."','".$this->array_table_data[$i]['Category']."','".$this->array_table_data[$i]['ItemID']."','".$this->array_table_data[$i]['ItemName']."','".$this->array_table_data[$i]['Description']."','".$this->array_table_data[$i]['Qty']."','".$this->array_table_data[$i]['Unit']."','".$this->array_table_data[$i]['ItemCode']."','".$this->v_gate_pass_date."','".$this->company_id."','".$this->company_name."','".$this->v_project_id."','".$this->v_project_name."','IssueNote', '".$data['msg']."', '".$data['msg1']."'),";  
		    ...
		    break;
		     ...
		     break;
		     case 'generate_gp_for_gate_pass': 
    			$this->msg = $this->varModelObj->ExecuteProcedureReturnMultiplevalues($var[18]);
    			$data = json_decode($this->msg, true);
    
    			 $SQLString = "INSERT INTO gate_pass_child_tbl(cat_id, cat_name, inventory_id, inventory_name, description, quantity, unit, item_code, pass_no, gate_pass_id) VALUES ";
    				  $SQLOther = "";
    			    for($i=0;$i<=sizeof($this->array_table_data)-1;$i++){
    			
    			      $SQLOther = $SQLOther ."('".$this->array_table_data[$i]['CategoryID']."','".$this->array_table_data[$i]['Category']."','".$this->array_table_data[$i]['ItemID']."','".$this->array_table_data[$i]['ItemName']."','".$this->array_table_data[$i]['Description']."','".$this->array_table_data[$i]['Qty']."','".$this->array_table_data[$i]['Unit']."','".$this->array_table_data[$i]['ItemCode']."','".$data['msg']."', '".$data['msg1']."'),";  
