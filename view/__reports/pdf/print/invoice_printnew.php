...
function convertThreeDigitGroupToWords($number) {
    ...
    while($row=mysqli_fetch_assoc($result)) {
		...
		$parts = explode('-', $invoice_real_no);
        if (count($parts) === 3) {
            $subParts = explode('/', $parts[2]);
            if (count($subParts) === 2) {
                $invoice_real_no = $parts[0] . '-' . $parts[1] . '-' . $subParts[1] . '/' . $subParts[0];
            }
        }
        ...
        $parts = explode('-', $intern_app_ref);
        if (count($parts) === 3) {
            $subParts = explode('/', $parts[2]);
            if (count($subParts) === 2) {
                $intern_app_ref = $parts[0] . '-' . $parts[1] . '-' . $subParts[1] . '/' . $subParts[0];
            }
        }
        $discount = 0;
