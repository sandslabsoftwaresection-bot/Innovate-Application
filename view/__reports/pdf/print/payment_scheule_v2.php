...
$pdf->setTitle('SAPPHIER/FUND_FLOW_STATEMENT/'.$invoice_number);
...
$pdf->AddPage();
 ...
 $array = explode('-', $_GET['v_customer_id']);
    if($array[1]==0 && $_GET['bank']=='empty'){
      $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' order by date_format(due_date,'%Y-%m-%d')");
//echo "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' order by date_format(due_date,'%Y-%m-%d')";
        ...
        $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' and customer_id='".$_GET['v_customer_id']."' order by date_format(due_date,'%Y-%m-%d')");
    }
    else if($array[1]==0 && $_GET['bank']!='empty')
    {
        if($_GET['bank']=='ALL'){
             $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' and bank_name!='' order by date_format(due_date,'%Y-%m-%d')");
        }
        else{
             $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' and bank_name='".$_GET['bank']."' order by date_format(due_date,'%Y-%m-%d')");
        }
    ...
    {
        if($_GET['bank']=='ALL'){
             $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' and bank_name!='' and customer_id='".$_GET['v_customer_id']."' order by date_format(due_date,'%Y-%m-%d')");
        }
        else{
             $result = mysqli_query($conns, "Select *,date_format(due_date,'%d-%b-%Y') as due_date from tlb_income_and_expence where date_format(due_date,'%Y-%m-%d') between '".$_GET['v_start_date']."' and '".$_GET['v_end_date']."' and bank_name='".$_GET['bank']."' and customer_id='".$_GET['v_customer_id']."' order by date_format(due_date,'%Y-%m-%d')");
        }
...
foreach ($result as $row_result) {
    if ($row_result['type'] == 'Expenditure') {
        ...
        $content_expense .= '</tr>';
        $exp_total = $exp_total +number_format((float)$row_result['dr_amount'], 3, '.', '')  ;
         $ctr++;
    } else {
        $result_receipt = ($row_result['cheque_no_receipt_no'] == '') ? '0000' : $row_result['cheque_no_receipt_no'];
        ...
        $content_income .= '</tr>'; 
        $income_total= $income_total + number_format((float)$row_result['cr_amount'], 3, '.', '');
        $ctr1++;
    }
  ...
  <tbody>
    <tr>
      <td width="50%"><table width="100%" border="1" style="border-collapse: collapse;" cellpadding="5" style="$table_border">
        <tbody>
          <tr>
            <td colspan="5" align="center" valign="middle" style="color:red;" bgcolor="$table_title_bg" style="$table_title_style">INCOME</td>
            </tr>
            <tr>
      ...
      <td width="50%"><table width="100%" border="1" style="border-collapse: collapse;" cellpadding="5" style="$table_border">
        <tbody>
          <tr>
            <td colspan="5" align="center" valign="middle" style="color:red;" bgcolor="$table_title_bg" style="$table_title_style">EXPENSE</td>
            </tr>
            <tr>
                <td width="8%" align="center" valign="middle" style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';" ><strong>Sl</strong></td>
	...
	<tr>
      <td><table width="100%" border="1" style="border-collapse: collapse;" cellpadding="5" style="$table_border">
        <tbody>
          <tr>
            <td width="58%" align="left" valign="middle" style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';" >Total Income</td>
