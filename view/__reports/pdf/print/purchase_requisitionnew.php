...
// 	$content .= '<table>';					
                            ...
                            $ctr = 1;
$amt = 0;
$notes = []; // collect all non-empty descriptions
...
while ($child_row = mysqli_fetch_assoc($pr_child_table)) {
    ...
    $full_inventory_name = $child_row['inventory_name'];
    $parts = explode('*', $full_inventory_name);
    // Table row
    ...
    $content .= '<td style="text-align: left;'.$valign_middle.$border_bottom.$border_left.';">'.$parts[0].'</td>';
    $content .= '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.$child_row['quantity'].'</td>';
    $content .= '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.$child_row['unit'].'</td>';
    $content .= '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($v_rate, 3).'</td>';
    $content .= '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.$v_tax.'</td>';
    $content .= '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($v_amount, 3).'</td>';
    $content .= '</tr>';

    $amt += $v_amount;
    $ctr++;

    // Collect note if description is not empty
    if (!empty(trim($child_row['description']))) {
        $notes[] = $child_row['description'];
    }
}

// Total amount row
$content .= '<tr style="border-bottom: 1px solid gray;">';
$content .= '<td colspan="6" style="text-align:left;'.$border_bottom_line.';"><strong>Total Amount</strong></td>';
$content .= '<td style="text-align: right;'.$border_left.$border_right.$border_bottom_line.';"><strong>';
$content .= ($amt == 0) ? '' : number_format($amt, 3);
$content .= '</strong></td>';
$content .= '</tr>';

// Close the table if it was opened earlier
// $content .= '</table>';

// Notes section (outside the table, no borders)
// if (!empty($notes)) {
//     $content .= '<div style="margin-top: 20px; text-align: left;">';
//     $content .= '<strong>Notes:</strong>';
//     $content .= '<ol style="padding-left: 20px; margin-top: 5px;">';

//     foreach ($notes as $note) {
//         $content .= '<li>' . htmlspecialchars($note) . '</li>';
//     }
...
if (!empty($notes)) {
    $td_note .= '<tr>';
    $td_note .= '<td colspan="6" style="text-align: left;' . $para_justify . '">';
    $td_note .= '<strong>Notes:</strong>';
    $td_note .= '<ol style="padding-left: 20px; margin-top: 5px;">';

    foreach ($notes as $note) {
        $td_note .= '<li>' . htmlspecialchars($note) . '</li>';
    }
...
$html = <<<EOD
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="4" align="center" id="main_table" style="$table_background">
        <tbody>
			...
			<br>
			<br>
		        <div class="signature-area" style="position: absolute; margin-bottom: 50px; width:100%; text-align:left;padding-top:50px;">
					<table class="tbl90" width="100%" border="0" cellspacing="0" cellpadding="5" align="left" style="$table_background">
					  <tbody>
            			...
            			<div style="padding-top:1px;">
            				<tr>
						<td><span><span style="text-align: left">&nbsp;&nbsp;Requested By : &nbsp;&nbsp;<strong> $requested_by </strong></span></span></td>
