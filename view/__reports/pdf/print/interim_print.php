...
function convertThreeDigitGroupToWords($number) {
    ...
    while($row=mysqli_fetch_assoc($result)) {
        ...
        if (count($parts) === 3) {
            ...
            if (count($subParts) === 2) {
                $invoice_number = $parts[0] . '-' . $parts[1] . '-' . $subParts[1] . '/' . $subParts[0];
            }
        }
             ...
             $val=  $total_discount / $total_quantity  ;
                 ...
                 $result = mysqli_query($conns,"SELECT  `invoice_child_id`, `delivery_note_child_id`, `invoice_no`, `quotation_child_id`, `delivery_note_no`, `quotation_no`, `description`, `quantity`, `unit`, `rate`, `amount`,`discount_type`, `discount_precentage`, `discount_amount`, `vat_percentage`,`net_amount` as net_amount, `default_date`, `quotation_status`, `required_quantity`,`purchase_qty`,`purchase_amount_pr`,supplied_amount_pr from intern_payment_child_tbl where invoice_no = '".$_GET['invoice_number']."'");
                     ...
                     while($row=mysqli_fetch_assoc($result)) {
				    	...
				    	$content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($executed_work).'%</td>';
				    	$content .=  '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($cum_upto_this,3).'</td>';	 
				    	$content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($supplied_amount_pr_inchild).'%</td>';
				    	$content .=  '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($last_period,3).'</td>';	 
				    	$content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($purchase_amount_pr_inchild).'%</td>';	 
