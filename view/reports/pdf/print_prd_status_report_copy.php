...
$pdf->AddPage();
            ...
            // echo 
                $result_project_repport = mysqli_query($conns,"SELECT COUNT(local_po_number) AS po_count, 
                        ...
                        ) and local_po_status!='Cancelled' and company_id IN (".$_GET['supplier'].") and cancel_on_demand!='Deactive'
                ...
                while($row=mysqli_fetch_assoc($result_project_repport)) {
                    ...
                    $company_id=$row['company_id'];
                     $total_amount=$row['total_amount'];
                                ...
                                $content .=  '<td colspan="2" style="text-align: center;'.$main_bg_color.$valign_middle.$border_bottom.$border_left.';">'.$po_count.'</td>';
                                $content .=  '<td colspan="2" style="text-align: center;'.$main_bg_color.$valign_middle.$border_bottom.$border_left.';">'.number_format($total_amount,3).'</td>';
                                $content .='</tr>';	
                                                
					$i++;				  
                
                    
                    $result_child_1 = mysqli_query($conns,"SELECT 
                            local_po_number, 
                            UPPER(company_name) AS company_name,
                            company_id, 
                            sub_total, 
                        ...
                        WHERE 
                            ...
                            AND company_id = '".$company_id."' AND cancel_on_demand!='Deactive'");  
                                
                                
                             	$content .='<tr nobr="true" style="'.$border_bottom.';">';
                                $content .=  '<td colspan="1"  style="text-align: center;'.$valign_middle.$border_bottom.$border_left.$child1_head_colour.';">S/N</td>';
                                $content .=  '<td colspan="5" style="text-align: left;'.$valign_middle.$border_bottom.$border_left.$child1_head_colour.';">LPO Number</td>';
                               $content .=  '<td colspan="2" style="text-align: center;'.$valign_middle.$border_bottom.$border_left.$child1_head_colour.';">Sub Total</td>';
                                $content .=  '<td colspan="2" style="text-align: center;'.$valign_middle.$border_bottom.$border_left.$child1_head_colour.';">Recieved Amount</td>';
                                $content .='</tr>';	   
                                ...
                                $content .=  '<td colspan="5" style="text-align: left;'.$child1_bg_color.$valign_middle.$border_bottom.$border_left.';">'.$local_po_number.'</td>';
                               $content .=  '<td colspan="2" style="text-align: center;'.$child1_bg_color.$valign_middle.$border_bottom.$border_left.';">'.number_format($sub_total,3).'</td>';
                                $content .=  '<td colspan="2" style="text-align: center;'.$child1_bg_color.$valign_middle.$border_bottom.$border_left.';">'.number_format($recieved_amount,3).'</td>';
                                $content .='</tr>';	
					        ...
					        $result_child_2 = mysqli_query($conns,"SELECT local_po_child_id,local_po_no, description,category_name, quantity,unit,rate,amount,vat_percentage,net_amount,quantity_purchased,cancel_ondemand_ch, (sum(quantity)-sum(quantity_purchased)) as balance,(select cancel_on_demand from local_po_main_tbl where local_po_number='".$local_po_number."')as cancel_status  FROM local_po_child_tbl WHERE local_po_no='".$local_po_number."' group by local_po_child_id");  
                              $content .= '<tr nobr="true" style="' . $border_bottom . ';">';
                            $content .= '<td style="width:5%;text-align: center;' . $valign_middle . $border_bottom . $border_left . $child2_head_colour . ';">S/N</td>';
                            $content .= '<td style="width:20%;text-align: left;' . $valign_middle . $border_bottom . $border_left . $child2_head_colour . ';">Item Name</td>';
                            $content .= '<td style="width:10%;text-align: center;' . $valign_middle . $border_bottom . $border_left . $child2_head_colour . ';">Quantity</td>';
                            $content .= '<td style="width:5%;text-align: center;' . $valign_middle . $border_bottom . $border_left . $child2_head_colour . ';">Unit</td>';
                            $content .= '<td style="width:10%;text-align: center;' . $valign_middle . $border_bottom . $border_left . $child2_head_colour . ';">Rate</td>';
					        ...
					        while ($row2 = mysqli_fetch_assoc($result_child_2)) {
                                ...
                                $content .= '<tr nobr="true" style="' . $border_bottom . ';">';
                                $content .= '<td style="width:5%;text-align: center;' . $valign_middle . $border_bottom . $border_left . ';">' . $k . '</td>';
                                $content .= '<td style="width:20%;text-align: left;' . $valign_middle . $border_bottom . $border_left . ';">'.$description.'</td>';
                                $content .= '<td style="width:10%;text-align: center;' . $valign_middle . $border_bottom . $border_left . ';">'.$quantity.'</td>';
                                $content .= '<td style="width:5%;text-align: center;' . $valign_middle . $border_bottom . $border_left . ';">'.$unit.'</td>';
                                $content .= '<td style="width:10%;text-align: center;' . $valign_middle . $border_bottom . $border_left . ';">'.$rate.'</td>';
  ...
  <tbody>
          ...
          <tr>
            ...
            <td align="left" valign="top" >
				<table width="100%"  cellspacing="0" cellpadding="5">
					<tbody>
						<tr >
						  <td style="text-align: center; color: #FFFFFF; font-size: 26px; " bgcolor="$table_title_bg">PENDING REPORT</td>
						</tr>
    ...
    <tr>
      <td>
        ...
        <tbody>
          <tr >
            <td colspan="1" width="9%" bgcolor="$table_title_bg" style="$table_title_style"><strong >S/N</strong></td>
            <td colspan="5" width="50%" bgcolor="$table_title_bg" style="$table_title_style"> <strong>Company Name</strong></td>
            <td colspan="2" width="20%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Count</strong></td>
              <td colspan="2" width="20%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Total Amount</strong></td>
