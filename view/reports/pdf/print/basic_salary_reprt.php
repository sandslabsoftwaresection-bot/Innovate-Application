...
$pdf->AddPage();
            ...
            //   $enddate=date("d-M-Y", strtotime($_GET['end']));
                $result_project_repport = mysqli_query($conns,"SELECT e.ids, e.employee_Name, e.employee_id, e.division_name, e.dpmt_name, COALESCE(s.earning_amt, '0.000') AS earning_amt 
...
WHERE e.status = 'Active'");  
                ...
                while($row=mysqli_fetch_assoc($result_project_repport)) {
                    $ids=$row['ids'];
                    $emp_name=trim($row['employee_Name']);
                    $emp_id=$row['employee_id'];
                    $division=trim($row['division_name']);
                     $department=trim($row['dpmt_name']);
                     $earning_amt=$row['earning_amt'];
                                ...
                                $content .=  '<td style="text-align: left;'.$valign_middle.$border_bottom.$border_left.';">'.$emp_name.'</td>';
                                $content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.$emp_id.'</td>';
                                $content .=  '<td style="text-align: left;'.$valign_middle.$border_bottom.$border_left.';">'.$division.'</td>';
                                $content .=  '<td style="text-align: left;'.$valign_middle.$border_bottom.$border_left.';">'.$department.'</td>';
                                $content .=  '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($earning_amt,3).'</td>';
                                $content .='</tr>';	
                                                
					$i++;	
					$sal_sum=$sal_sum+$earning_amt;
                }                        
                $sal_sum=number_format($sal_sum,3);
          ...
          <tr>
            ...
            <td align="left" valign="top" >
				<table width="100%"  cellspacing="0" cellpadding="5">
					<tbody>
						<tr >
						  <td style="text-align: center; color: #FFFFFF; font-size: 26px; " bgcolor="$table_title_bg">SALARY REPORT</td>
						</tr>
      ...
      <td>
        ...
        <tbody>
          <tr >
            ...
            <td width="22%" bgcolor="$table_title_bg" style="$table_title_style"> <strong>Employee Name</strong></td>
            <td width="13%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Employee ID</strong></td>
              <td width="22%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Division</strong></td>
              <td width="21%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Department</strong></td>
            <td width="16%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Salary</strong></td>
		...
		$content
		    <tr >
		        ...
		        <td style="text-align: right;border-left: 1px solid #09037f"><strong>$sal_sum</strong></td>
            </tr>
		</tbody>
	    </table>
	  </td>
