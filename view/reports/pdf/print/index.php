...
// Create connection
   ...
   $conns = $db_con->ConnectToMYSQL();
    ...
    while($row=mysqli_fetch_assoc($result)) {
        ...
        $invoice_date = date("d-M-Y", strtotime($row['quotation_date']));
        ...
        $total_amount = trim($row['sub_total']) ;
        $received_by_id = trim($row['received_by_id']) ;
        ...
        $total_discount_amount = trim($row['total_discount_amount']) ;
        $tax_content= trim($row['tax_content']);
        $discount = 0;
...
$pdf->setTitle('SAPPHIRE/QNO/'.$_GET['quotation_number']);
...
$pdf->AddPage();
                 ...
                 $result = mysqli_query($conns,"select * from quotation_child_tbl where quotation_no = '".$_GET['quotation_number']."'");
                     while($row=mysqli_fetch_assoc($result)) {
                         ...
                         if($row['discount_amount']!=0)
                         {
                          ...
                          if (trim($row["product_name"]) != "-Select Product--") {
                                $content .= '<td style="text-align: left;'.$para_justify.$border_bottom.$border_left.';">
                                        <strong><u>'.trim($row["product_name"]).'</u></strong> '.trim($row["description"]).'
                                    </td>';
                            }
                        else
                        {
                          ...
                          $content = $content.'  <td  style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($row["rate"],3).'</td>';
                ...
                //$content = $content.'    <td style="text-align: center; '.$border_bottom_line.'">&nbsp;</td>';
                    
                $content = $content.'    <td colspan="5" style="text-align: left;'.$border_bottom_line.' "><strong>Total Amount in BHD</strong></td>';
                   
                $content = $content.'    <td style="text-align: right;'.$border_left.';'.$border_bottom_line.'" colspan="1"><strong>';
                $amt2=number_format($amt,3);
                $content = $content.$amt2;
                ...
                $content = $content.'    <td colspan="5" style="text-align: left;'.$border_bottom_line.'"><strong>Discount Amount</strong></td>';
                   
                $content = $content.'    <td style="text-align: right;'.$border_left.';'.$border_bottom_line.'" colspan="5"><strong>'.number_format($total_discount_amount+$discount,3).'</strong></td>';
                $content = $content.'  </tr>';
                $content = $content.'   <tr style="border-bottom: 1px solid gray;">';
                //$content = $content.'    <td style="text-align: center;'.$border_bottom_line.'">&nbsp;</td>';
                    
                $content = $content.'    <td colspan="5" style="text-align: left;'.$border_bottom_line.'"><strong>Balance Amount</strong></td>';
                ...
                $content = $content.'    <td colspan="5" style="text-align: left;'.$border_bottom_line.'"><strong>VAT (';
                ...
                $content = $content.'    <td colspan="5" style="text-align: left;'.$border_bottom_line.'"><strong>Grand Total BHD</strong></td>';
                ...
                $content = $content.'  <tr >';
                $content = $content.'    <td colspan="10" style="text-align: center; font-size:12px;"><strong>Bahraini Dinars: ';
...
<table width="100%" border="0" cellspacing="0"   id="main_table" style="$table_background">
  ...
  <tbody>
    <tr >
      <td style="padding-left: 10px;padding-right: 10px">
        ...
        <tbody>
          <tr>
            ...
            <td align="left" valign="top" >
				<table width="100%"  cellspacing="0" cellpadding="5">
					<tbody>
						<tr >
						  <td style="text-align: center; color: #FFFFFF; font-size: 26px; " bgcolor="$table_title_bg">QUOTATION </td>
						</tr>
          ...
          <tr>
            <td colspan="2" align="left"  style="height: 30px;"><strong>SUBJECT :  $quotation_reference  </strong></td>
          </tr>
          <tr>
            <td colspan="2" align="left"  style="height: 60px;$para_justify">Dear Sir, <br>$subject </td>
          </tr>
        </tbody>
      </table>
      ...
      <td >
        ...
        <tbody>
          <tr >
            ...
            <td bgcolor="$table_title_bg" style="width:48%;$table_title_style"> <strong>Description</strong></td>
    ...
    <tr>
        <td>
                <table width="100%" border="0" cellspacing="0"  align="center" style="$table_background">
                  <tbody>
                    <tr>
                      <td><span style="margin-left:10px;padding-left: 10px;padding-right: 10px;text-align: left">$description </span></td>
                    </tr>
                  </tbody>
                </table> 
        </td>
...
$download_file_name=str_replace("/","-",$invoice_number).'.pdf';
//Close and output PDF document
$pdf->Output($download_file_name, 'I');
