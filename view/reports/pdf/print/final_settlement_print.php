...
// Create connection
   ...
   $conns = $db_con->ConnectToMYSQL();
    ...
    while($row=mysqli_fetch_assoc($result)) {
       ...
       $rejoining_date=date("d-M-Y", strtotime($row['rejoining_date']));
       $ending_date=date("d-M-Y", strtotime($row['ending_date']));
        $travel_allo_rate = $row['travel_allo_rate'];
        $indemnity_rate = $row['indemnity_rate'];
        $leave_months = $row['leave_months'];
     
        // $v_rejoining = new DateTime($row['rejoining_date']);
        // $v_last = new DateTime($row['ending_date']);
  
    } 
        
        
//         // Calculate the difference in days
//       $differenceSeconds = abs($v_last->getTimestamp() - $v_rejoining->getTimestamp());

// // Convert seconds to days
// $differenceDays = ceil($differenceSeconds / (60 * 60 * 24));
//     $daysPerYear = 365;
// $daysPerMonth = 30.44;
// $remainingDays = $differenceDays % $daysPerYear;

// // Calculate the number of months
// $months = floor($differenceDays / $daysPerMonth);
// $remainingDays %= $daysPerMonth;
// $remainingDays=$remainingDays;
//         $result = $months . '.' . $remainingDays;
...
$pdf->setTitle('SAPPHIER/Final_Settlement/'.$emp_name);
...
$pdf->AddPage();
                 ...
                 $result = mysqli_query($conns,"select * from  salary_child_tbl where main_tbl_id = '".$_GET['main_id']."'");
                     ...
                     while($row=mysqli_fetch_assoc($result)) {
            				...
            				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
                				...
                				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
                						...
                						$ded_tot=$ded_tot+$deduction_amt;
						                 ...
						                 $content_ded = $content_ded . '<tr nobr="true">';
                				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
                						$content_ded = $content_ded . '<td bgcolor="#ebe9e4" style="text-align: left; line-height: 20px; border-right: 1px solid #09037f;">' . $allow_deduction . ' ('.$leave_days_taken.' days)</td>';
                						$content_ded = $content_ded . '<td bgcolor="#ebe9e4" style="text-align: right; line-height: 20px;">' . number_format($deduction_amt,3) . '</td>';
                						$content_ded = $content_ded . '</tr>';
                						$ded_tot=$ded_tot+$deduction_amt;
						            }
                				...
                				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
                        				...
                        				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
						                    ...
						                    else if($allo_ded_tbl_id==2)
						                    {
						                        $content_ded = $content_ded . '<tr nobr="true">';
                        				// 		$content = $content . '<td bgcolor="#f2f2f2" style="text-align: center; border-right: 1px solid black; border-left: 1px solid #000;">' . $i . '</td>';
                        						$content_ded = $content_ded . '<td bgcolor="#ebe9e4" style="text-align: left; border-top: 1px solid #09037f; line-height: 20px; border-right: 1px solid #09037f;">' . $allow_deduction . ' ('.$leave_days_taken.' days)</td>';
                        						$content_ded = $content_ded . '<td bgcolor="#ebe9e4" style="text-align: right; border-top: 1px solid #09037f; line-height: 20px;">' . number_format($deduction_amt,3) . '</td>';
                        						$content_ded = $content_ded . '</tr>';
                        						$ded_tot=$ded_tot+$deduction_amt; 
						                    }
                      ...
                      if($allo_ded_tbl_id==2 && $status=="Allowence"){
                        ...
                        if($allo_ded_tbl_id==4 && $status=="Allowence"){
                            $travel_days=$leave_days;
                            $travel_amt=$earning_amt;
                        }
                        if($allo_ded_tbl_id==5 && $status=="Allowence"){
                            $indemnity_days=$leave_days;
                            $indemnity_amt=$earning_amt;
                        }
                        $travel_and_inde_sum=$travel_amt+$indemnity_amt;
	  ...
	  if($normal_hrd=="")
	    ...
	    $travel_amt=number_format($travel_amt,3);
	    $indemnity_amt=number_format($indemnity_amt,3);
	    $travel_and_inde_sum=number_format($travel_and_inde_sum,3);
	    $word= getCurrency($total_amt);
	   // $total_months=number_format($total_months,3);
...
<table width="100%" border="1" cellpadding="3" style="border-collapse: collapse;">
  <tbody>
    <tr style="$style_padding">
      <td  colspan="4" align="center" bgcolor="#09037f" style="color: white;"><b>Pay Slip For Final Settlement ($rejoining_date to $ending_date)</b></td>
    </tr>
    <tr>
    ...
    <tr style="padding: 0px;">
      <td colspan="4" style="padding: 0px;">
			...
			<table width="100%" cellpadding="3" border="1" style="border-collapse: collapse; text-align: right;">
		  		...
		  		<tr>
				  <td colspan="4" align="center" bgcolor="#09037f" style="color: white;"><b>Overtime Summary</b></td>
				</tr>
				<tr>
    ...
    <tr style="padding: 0px;">
      <td colspan="4" style="padding: 0px;">
			...
			<table width="100%" cellpadding="3" border="1" style="border-collapse: collapse; text-align: right;">
		  		...
		  		<tr>
				  <td colspan="4" align="center" bgcolor="#09037f" style="color: white;"><b>Final Settlement Summary</b></td>
				</tr>
				<tr>
				  <td align="center" style="$border_colour">Type</td>
				  <td align="center" style="$border_colour">Months</td>
				  <td align="center" style="$border_colour">Amount </td>
				  <td align="center" style="$border_colour">Net Amount </td>
				</tr>
				<tr>
				  <td align="center" style="$border_colour">Travel Allowance</td>
				  <td align="center" style="$border_colour">$total_months</td>
				  <td align="right" style="$border_colour">$travel_amt</td>
				  <td align="center" rowspan="2" style="font-size:18px; $border_colour">$travel_and_inde_sum</td>
				</tr>
				<tr>
				  <td align="center" style="$border_colour">Indemnity Bonus</td>
				  <td align="center" style="$border_colour">$total_months</td>
				  <td align="right" style="$border_colour">$indemnity_amt</td>
				   
				</tr>
				<tr>
				  <td colspan="4" align="left" style="$border_colour">Note $note</td>
				  
				</tr>
				<tr>
				  <td colspan="4" align="left" style="$border_colour">This is a system-generated salary slip; no signature or seal is required. </td>
				</tr>
				<tr >
				  <td colspan="4" align="right" style="$border_colour">Authorized Signatory </td>
			    </tr>
		  </table>
