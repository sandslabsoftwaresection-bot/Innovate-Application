...
// Create connection
   ...
   $conns = $db_con->ConnectToMYSQL();
    ...
    while($row=mysqli_fetch_assoc($result)) {
        ...
        $pass_in_date = date("d-M-Y", strtotime($row['pass_in_date']));
        ...
        $v_project_id = trim($row['project_id']) ;
...
$pdf->AddPage();
              ...
              $i=1;
                 $result = mysqli_query($conns,"Call proc_list_passin_for_edit('".$pass_in_no."','".$v_project_id."',@msg)");
                     while($row=mysqli_fetch_assoc($result)) {
                        $inventory_id=$row['inventory_id'];
                        $inventory_name=$row['inventory_name'];
                        $description=$row['description'];
                        // $qty=$row['current_return_qty'];
                        $qty = $row['current_return_qty'];
                        $qty_array = explode(',', $qty);
                        $damage_qty=$row['current_damage_qty'];
                        $damg_qty_array = explode(',', $damage_qty);
                        $damg_qty_array[1]='<span style="color:red;">' . $damg_qty_array[1] . '</span>';
                        $merged_description  =$qty_array[1].'<br>'. $damg_qty_array[1] ;
                        if($damg_qty_array[1]=="")
                        {
                            $merged_description = $qty_array[1];
                        }
                        
                        if($qty_array[1]=="")
                        {
                            $merged_description = $damg_qty_array[1];
                        }
                        $unit=$row['unit'];
		                ...
		                if($damg_qty_array[0]==""){
		                    ...
		                    $damage_qty=$damg_qty_array[0];
		                }
						 
                         if($qty_array[0]!="" || $damg_qty_array[0]!=""){
                                            ...
                                            $content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($ret_qty, 2).'</td>';
                                            $content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.'; color: red;">'.number_format($damage_qty, 2).'</td>';
                                            $content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.$unit.'</td>';
                                            $content .=  '<td style="text-align: left;'.$valign_middle.$border_bottom.$border_left.';">'.$merged_description.'</td>';
                                            $content .='</tr>';
                                            	$i++;
                         }     
          ...
          <tr>
            ...
            <td align="left" valign="top" >
				<table width="100%"  cellspacing="0" cellpadding="5">
					<tbody>
						<tr >
						  <td style="text-align: center; color: #FFFFFF; font-size: 26px; " bgcolor="$table_title_bg">PASS IN</td>
						</tr>
      ...
      <td>
        ...
        <tbody>
          <tr >
              ...
              <td  width="10%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Damaged Qty</strong></td>
              <td width="10%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Unit</strong></td>
              <td  width="35%" bgcolor="$table_title_bg" style="$table_title_style"><strong>Remarks</strong></td>
...
$pass_in_no = str_replace("/","-",$pass_in_no);
$pdf->Output($pass_in_no.'.pdf', 'I');
