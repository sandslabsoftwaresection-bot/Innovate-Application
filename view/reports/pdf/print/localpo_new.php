...
// Create connection
   ...
   $conns = $db_con->ConnectToMYSQL();
    ...
    while($row=mysqli_fetch_assoc($result)) {
     ...
     $result_supplier = mysqli_query($conns,"select * from supplier_details  where 	company_id ='".$company_id."'");
                ...
                $tot_amt = 0;
                 $result = mysqli_query($conns,"select * from local_po_child_tbl where local_po_no = '".$_GET['local_po_number']."'"); 
                     while($row=mysqli_fetch_assoc($result)) {
                                            ...
                                            $content .=  '<td style="text-align: center;'.$para_justify.$border_bottom.$border_left.';">'.$row['description'].'</td>';
                                            ...
                                            $content .=  '<td style="text-align: center;'.$valign_middle.$border_bottom.$border_left.';">'.$row['unit'].'</td>';
                                          	$content .=  '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($row["discount_amount"]/$row["quantity"],3).'</td>';
                                       		$content .=  '<td style="text-align: right;'.$valign_middle.$border_bottom.$border_left.';">'.number_format($row["discount_amount"],3).'</td>';
                                            
						                    $amt=$amt+$row["net_amount"]; 
                                            $tot_amt=$tot_amt+$row["discount_amount"];
                                            $vat_amt = $vat_amt+ ($row["discount_amount"]*$row['vat_percentage']/100);
                    						$vat_percentage = number_format($row['vat_percentage'],0);    	  
                                              $content = $content.'</tr>';
                                          $ctr = $ctr +1;  
												 
                                      }
									   $content .='<tr style="border-bottom: 1px solid gray;">';
                        			   $content .='<td colspan="5" style="text-align: left;'.$border_bottom_line.' "><strong>Total Amount in BHD</strong></td>';
                        			   $content .='<td style="text-align: right;'.$border_left.';'.$border_bottom_line.';"><strong>';
                        			   
                        			  $tot_amt=number_format($tot_amt,3);
                                      $content = $content.$tot_amt;
                                      $content = $content.'</strong></td>';
                                      $content .='</tr>';
                                      
                                      $content .='<tr style="border-bottom: 1px solid gray;">';
                        			  $content .='<td colspan="5" style="text-align: left;'.$border_bottom_line.' "><strong>VAT ('.$vat_percentage.'%) in BHD </strong></td>';
                        			  $content .='<td style="text-align: right;'.$border_left.';'.$border_bottom_line.';"><strong>';
                                      $vat_amt=number_format($vat_amt,3);
                                      $content = $content.$vat_amt;
                                      $content = $content.'</strong></td>';
                                      $content .='</tr>';
                                      
                                      $content .='<tr style="border-bottom: 1px solid gray;">';
                        			  $content .='<td colspan="5" style="text-align: left;'.$border_bottom_line.' "><strong>Net Amount in BHD</strong></td>';
                        			  $content .='<td style="text-align: right;'.$border_left.';'.$border_bottom_line.';"><strong>';
                                      $amt_format=number_format($amt,3);
                                      $content = $content.$amt_format;
                                      $content = $content.'</strong></td>';
                                      
                        			   $content .='</tr>';
                        			   
                        			    $content = $content.'  <tr >';
                                        $content = $content.'    <td colspan="8" style="text-align: center; font-size:12px">Bahraini Dinars: <strong>';
