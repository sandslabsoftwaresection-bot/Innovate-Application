			  ...
			  <!--*** CONTENT GOES HERE ***-->
				...
				<table width="800" border="0" cellspacing="0" cellpadding="5" align="center" id="main_table">
				  <tbody>
						  ...
						  <tr>
							...
							<td align="left" valign="top" >
								<table width="370px"  cellspacing="0" cellpadding="5" align="right">
								  <tbody>
									<tr >
									  <td style="text-align: center; color: #FFFFFF; font-size: 26px; font-family: 'Century Gothic';" bgcolor="#0079DD" align="right">FINISHED INVENTORY LIST</td>
									</tr>
					  ...
					  <td style="padding-left: 10px;padding-right: 10px">
						...
						<tbody>
						  ...
						  <?PHP 
						        $sum_qty_in = 0;
								$sum_qty_out = 0;
								$sql = "SELECT `finished_pdt_id` as inventory_id,`finished_pdt_name` as inventory_name,'' as description,`pdt_qty` as qty,`pdt_unit` as unit,`insert_date` as trans_date,'' as ref_no,
					'Manufactured In Factory And Deliver in Store2' as action,'FGST2' as type FROM `tbl_store2_finished_pdt_master` WHERE  finished_pdt_id= '".$_GET['row_ids']."'   UNION  SELECT `inventory_id` as inventory_id,
					...
					'GPOUT' as type FROM `view_gate_pass_inventory_out` WHERE inventory_id= '".$_GET['row_ids']."'  and (`parent_ids`!='' or `parent_ids` IS NOT NULL)UNION  SELECT `inventory_id` as inventory_id,`inventory_name` as inventory_name,
					description as description,`qty` as qty,`unit` as unit,`entry_date` as trans_date,ref_no as ref_no,'Return To Store2' as action,'PASSIN' as type FROM `view_pass_in_inventory_in` WHERE  inventory_id= '".$_GET['row_ids']."' and (`parent_ids`!='' or `parent_ids` IS NOT NULL) ORDER BY trans_date asc";
...
function fnExcelReport()
{
	...
	var filename = "Machinery list - "+item+".xls";
