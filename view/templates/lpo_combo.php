...
<?PHP 
   $result = mysqli_query($varDBConnection,"SELECT local_po_main_id,local_po_number FROM local_po_main_tbl WHERE `local_po_status`!='Cancelled' and `local_po_number` in (select local_po_no from local_po_child_tbl where (quantity-quantity_purchased)>0) group by `local_po_number` order by local_po_main_id desc ");
