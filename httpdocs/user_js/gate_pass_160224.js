$(document).ready(function(){
                ...
                var tbl_gate_pass_list = $('#tbl_gate_pass_list').DataTable({
                    ...
                    ordering: false,
                    columnDefs: [
                        { targets: [2, 4,5,8,9,10], visible: false } // Adjust column indices as needed
                    ]
                });
	...
	function load_data_to_grid_inventory_list(v_item_id,flag)
	 {
		 ...
		 tbl_inventory_list = $('#tbl_inventory_list').DataTable( {
				...
				"columns": [
					 ...
					 { "data": "Stock" , "width":'10%',className: "text-center","visible":false
    ...
    //                          }
             ...
             function view_gate_list(category,categoryid,item,itemid,itemCode,description,qty, stock, unit,childId)
                    ...
                    '<button class="btn btn-danger btn-sm delete-rows">Delete</button>' // Delete button
                        ...
                        $('#tbl_gate_pass_list').on('click', '.delete-rows', function() {
                            var row = $(this).closest('tr');
                    ...
                    '<button class="btn btn-primary btn-sm update-row">Update</button>' // Update button
                        ...
                        $('#tbl_gate_pass_list').on('click', '.delete-row', function() {
                            ...
                            swal({
								...
								.then((willDelete) => {
								  if (willDelete) {
                            $.post("../controller/gate_pass/gate_pass_controller.php",{action:"delete_child_gp",child_ids:v_child_ids},function(result,status){
                        ...
                        $('#tbl_gate_pass_list').on('click', '.update-row', function() {
                         ...
                         $('#tbl_gate_pass_list').on('click', '.save-row', function() {
                             var row = $(this).closest('tr');
                                ...
                                $.post("../controller/gate_pass/gate_pass_controller.php",{action:"update_child_gp", child_ids:child_ids, v_qty:qty, v_description:description},function(result,status){
											 ...
											 //alert(result);
											    if(result== 1)
												{
												  ...
												  swal("Success","Quantity updated successfully", "success");
                                                    ...
                                                    load_data_to_grid_second_table(master_id)
												}
                            });
             ...
             list_of_gate_pass_table.destroy();
                            ...
                            list_of_gate_pass_table = $('#list_of_gate_pass').DataTable({
                                     ...
                                     "ajax": {
                                         ...
                                         'url': '../controller/gate_pass/gate_pass_controller.php',
                                         'data': {
                                            action: 'search_with_date_new_gp',
...
$('#list_of_gate_pass tbody').on('click', 'td button', function (){
        ...
        //                 $("#select_iventory_category").trigger("chosen:updated");
						  ...
						  $("#txt_master_ref_id").val(v_ids);
						   load_data_to_grid_second_table(v_ids);
					  ...
					  swal({
								...
								.then((willDelete) => {
								  if (willDelete) {
										 $.post("../controller/gate_pass/gate_pass_controller.php",{action:"delete_gate_pass_main",v_pass_no:v_pass_no},function(result,status){
...
function load_data_to_grid_second_table(v_ids)
{
     $.post("../controller/gate_pass/gate_pass_controller.php",{action:'select_child_table_gate_pass',v_gate_pass_ids:v_ids},function(result,status){
                                ...
                                view_gate_list_for_edit(item.cat_name, item.cat_id, item.inventory_name, item.inventory_id, item.item_code, item.description, item.quantity, 0, item.unit, item.gate_pass_child_id);
                                $('#tbl_inventory_list').hide();
...
$("#btn_gate_pass_print_without_head").click(function()
			  {
               var pass_no=$('#txt_pass_no').val();
                    ...
                    if($.trim(pass_no)=="")
                    {
                       ...
                       {
                          window.open("reports/pdf/print/gp_updated.php?pass_no="+pass_no+"&x=1","_blank"); 
               ...
               var pass_no=$('#txt_pass_no').val();
                    ...
                    if($.trim(pass_no)=="")
                    {
                  ...
                  else
                       {
						  ...
						  window.open("reports/pdf/print/gp_updated.php?pass_no="+pass_no+"&x=0","_blank"); 
