$(document).ready(function(){
                 ...
                 var quatation_table_modal = $('#quatation_table_modal').DataTable( {searching: false, paging: false, info: false,"ordering": false});              
                 $('#quatation_view_div').hide();
...
<!------------------------------------QuotationNoLoad----------------------------------------------------->               
                ...
                $("#div_project_select_combo").change(function(){
                    var project_id = ($('option:selected', this).val()); 
                     ...
                     $.post("../controller/quotation_new/quotation_new_controller.php",{action:'select_vat_content',v_project_id:project_id},function(result,status){
                           
                           var obj= jQuery.parseJSON(result);
                          // $("#txt_tax_content").val(obj.data[0].tax_content); 
                           $("#txt_location").val(obj.data[0].project_number); 
                      });
...
$("#div_select_quotation_combo").change(function(){
    ...
    $("#work_order_view_div").hide();
    $('#quatation_view_div').show();
    quotation_modal_data(quotation_no);
...
//**************************quatation table modal Load ***************************
                    
                     function quotation_modal_data(quotation_no)
                    {
                             quatation_table_modal.destroy();
                                 
                             quatation_table_modal = $('#quatation_table_modal').DataTable({
                                    
                                     "ajax": {
                                         'type': 'POST',
                                         'url': '../controller/working_order/working_order_controller.php',
                                         'data': {
                                            action: 'quotation_modal_table',
                                    ...
                                    "columns": [
                                         ...
                                         { "data": "quotation_child_id",className: "text-center"},
                                         { "data": "description"},
                             ...
                             tbl_work_order_list = $('#tbl_work_order_list').DataTable({
                                    ...
                                    "columns": [
                                         ...
                                         {"data": "quotation_status",className: "text-center"},
                                          ...
                                          {"data": "work_order_child_id", width: "12%",
                                                  render: function ( data, type, rows, meta ) {
                        						
                        									order_action = ' <button type="button" class="btn btn-sm" style="background-color: #10911a;color: #fff;margin-left: -6px;margin-right:2px;" id="change_status" name="change_status" ><i class="material-icons">assignment_turned_in</i></button><button type="button" class="btn btn-sm primary-gradient mr-1"  id="edit_work_order" name="edit_work_order" ><i class="material-icons ">edit</i></button><button type="button" class="btn btn-sm btn-danger mr-1"  id="delete_work_order" name="delete_work_order"><i class="material-icons ">delete</i></button>';
            								                return order_action;
			...
			function load_data_to_grid_view_list_of_work_order()
			{
				 work_order_view_list_table.destroy();
                            ...
                            buttons: [
                                {
                                    extend: 'excelHtml5',
                                    title: 'WorkOrder_List',
                                    text: '<i class="material-icons">grid_on</i> Excel',
                                    className: 'btn btn-sm btn-success',
                                    exportOptions: {
                                        columns: [1, 2, 3,4]  // Export only these column indexes
                                    }
                                },
                                {
                                    extend: 'pdfHtml5',
...
$('#list_of_work_order tbody').on('click', 'td button', function (){
                       ...
                       var monthNumber = monthMap[dateParts[1]];
                       var woDate = dateParts[2] + '-' + monthNumber + '-' + dateParts[0];
 ...
 <!-----------------------------------Edit Work Order-------------------------------------------->               
				...
				swal("Warning","Please fill the required field", "warning");
				 ...
				 else
				 {
            		   ...
            		   $.post("../controller/working_order/working_order_controller.php",{action:'update_working_order',v_company_id:company_id,v_company_name:company_name,v_project_id:project_id,v_project_name:project_name,v_quotation_id:quotation_id,v_quotation_no:quotation_no,v_location_txt:location_txt,v_received:received,v_working_date:working_date,v_working_end_date:working_end_date,work_order_no:work_order_no,note_txt:note_txt},function(result,status){
                               $('#txt_work_order_no').val(result);          
