$(document).ready(function(){
             ...
             $("#txt_invoice_received_amount,#txt_invoice_retention_percentage,#txt_invoice_previous_bill_amount").change(function(){
                     ...
                     $.post('../controller/invoice/new_invoice_controller.php',{action:'select_total_discount_amount',v_quotation_no : quotation_no},function(result,status){
                         if(status=='success')
                         {
                            alert(result);
                              ...
                              // $('#txt_hidden_discount_type').val(obj.data[0].discount_type);
                              // $('#txt_invoice_discount_type').val(obj.data[0].discount_amount+ '('+obj.data[0].discount_type+')');
                              $("#txt_discount_amount_qt").val(obj.data[0].discount_amount+ '('+obj.data[0].discount_type+')');
                            ...
                            "columns": [
								 ...
								 { "data": "qty_change", width:"10%",
								  render: function ( data, type, rows, meta ) {
											...
											var grade_edit =' <input type="number"  id="txt_qty_change_'+rows["quotation_child_id"]+'" name="txt_qty_change" style="width:100px;">';
            					 ...
            					 { "data": "discount_precentage",className: "text-right",render: $.fn.dataTable.render.number(',', '.', 3, ''),
            					    render: function ( data, type, rows, meta ){
										...
										var grade_edit =' <input type="number" class="discount-input" id="txt_net_total_perc_'+rows["quotation_child_id"]+'" name="txt_net_total_perc" style="width:100px;">';
            					 ...
            					 { "data": "net_amount",className: "text-right",render: $.fn.dataTable.render.number(',', '.', 3, ''),
									render: function ( data, type, rows, meta ){
										...
										var grade_edit =' <input type="number" id="txt_net_total_'+rows["quotation_child_id"]+'" name="" value="'+rows["net_amount"]+'" style="width:100px;" disabled>';
				...
				$('#tbl_delivery_note_list tbody').on('change', 'td input', function() {
					...
					//txt_default_net_total = $('#txt_net_total').val();
					// alert(txt_default_net_total);
					...
					if($(this).attr("name")=='txt_qty_change')
					{
						...
						alert(v_quantity);
							...
							if(parseInt(v_txt_qty_change) <= parseInt(v_quantity))
							{
						...
						if (v_txt_qty_change !== '')
						{
							$('#txt_net_total_perc').prop('disabled', true);
						} 
						else 
						{
							//alert(txt_default_net_total);
							$('#txt_qty_change').val('');
							$('#txt_net_total').val(v_net_amount);
							$('#txt_net_total_perc').prop('disabled', false);
						}
					...
					if($(this).attr("name")=='txt_net_total_perc')
					{
						...
						v_txt_net_total_perc = $('#txt_net_total_perc').val();
						
						if(v_txt_net_total_perc != '')
						{
							$('#txt_qty_change').prop('disabled', true);
							...
							$('#txt_net_total').val(v_net_total_perc_due);
							
						}
						else
						{
							$('#txt_net_total').val(v_net_amount);
							$('#txt_net_total_perc').val('');
							$('#txt_qty_change').prop('disabled', false);
						}
					}
                ...
                $('#tbl_delivery_note_list tbody').on('click', 'td button', function (){
                        ...
                        v_discount_type=data.discount_type;
						 ...
						 {
							//alert(v_invoice_child_id);
							var v_invoice_no=$("#txt_invoice_no").val();
							if(v_txt_qty_change == '' && v_txt_net_total_perc != '')
							{
							//alert(v_net_total_due+"-"+v_txt_qty_change+"-"+v_net_total_perc_due);
							 $.post("../controller/invoice/invoice_controller.php",{action:'update_invoice_list_item',v_net_total_perc_due:v_net_total_perc_due,v_invoice_child_id:v_invoice_child_id}, function(result,status){
							...
							else if(v_txt_qty_change != '' && v_txt_net_total_perc == '')
							{
								//alert(v_net_total_due+"-"+v_txt_qty_change+"-"+v_net_total_perc_due);
								$.post("../controller/invoice/invoice_controller.php",{action:'update_qty_net_invoice_list_item',v_net_total_perc_due:v_net_total_due,v_txt_qty_change:v_txt_qty_change,v_invoice_child_id:v_invoice_child_id}, function(result,status){
