  ...
  $(document).ready(function() {
     ...
     var todaysDate = new Date().toISOString().split('T')[0];
	  ...
	  $('#tlbTransaction').on('click', '.sands-badge-grid', function(button) {
	               ...
	               var confirm_status = rowData['confirm_status'];
		           if(confirm_status=="Not Confirm"){
		               
        					var statusElement = $(this);
        				// 	 var eighthElement = closestRow.find('td:eq(8)'); 
        			        var eighthElement = closestRow.find('td:eq(8) .sands-delete-icon, td:eq(8) .checkmark_red_icon'); // Getting the div element within the 8th index column
        					...
        					$('#popupContainer').data('checkmarkRedIcon', eighthElement);
		           }
	    ...
	    function ChangeStatus()
	    {
                ...
                var eighthElement = $('#popupContainer').data('checkmarkRedIcon');
                // OK button clicked, perform action
                ...
                eighthElement.removeClass("sands-delete-icon checkmark_red_icon");
                eighthElement.addClass((newStatus === "Approved") ? "checkmark_red_icon" : "sands-delete-icon");
...
function DeleteRowFromTheTable()
{
	...
	// Confirm Items
         $('#tlbTransaction').on('click', '.checkmark_red_icon', function() {
...
function ConfirmRowFromTheTable()
{
        	    ...
        	    console.log(row);
        	     ...
        	     $.post("../controller/payment_schedule/payment_schedule_controller.php",{action:"confirm_row",v_rowId:row_Id},function(result,status){
                        ...
                        row.addClass('disabled-row');
                         ...
                         if (row instanceof jQuery) {
                                statusElementConfirm.removeClass('checkmark_red_icon').addClass('checkmark_green_icon'); // Replace 'old-class' with the actual class to remove
                            } else {
                                // If row is not a jQuery object, convert it
                                $(statusElementConfirm).removeClass('checkmark_red_icon').addClass('checkmark_green_icon'); // Replace 'old-class' with the actual class to remove
                            }
